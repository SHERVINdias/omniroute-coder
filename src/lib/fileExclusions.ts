/**
 * src/lib/fileExclusions.ts
 * ---------------------------------------------------------------------------
 * One place that decides whether the model is allowed to see a file.
 *
 * WHY THIS EXISTS
 *
 * There were three answers to that question and they disagreed:
 *
 *   1. `vscode-extension/src/fileOps.ts` had two hardcoded regex arrays
 *      (BLOCKED_DIRECTORY_PATTERNS / BLOCKED_FILE_PATTERNS) applied to reads
 *      and writes but NOT to its own directory listing.
 *   2. `src/lib/productionToolRegistry.ts` had a separate `.aiignore` loader,
 *      matched with minimatch, applied only to the Production-mode tools.
 *   3. `src/lib/vscodeBridge.ts` had no content rules at all — only
 *      `guardPath`, which checks the workspace BOUNDARY, not what is inside it.
 *
 * So which files were protected depended on which mode you were in and which
 * tool the model happened to reach for, and none of the three could be changed
 * without editing source. The user cannot be told "you have complete control
 * over what the LLM reads" while the answer is spread across three files, two
 * matching libraries and one of them skipping listings.
 *
 * This module is the single rule engine. It is deliberately:
 *
 *   PURE          no imports, no I/O, no DB, no VS Code API. It can be used on
 *                 the server, inside the extension, and in the browser (the
 *                 settings UI previews matches live), and it can be copied into
 *                 the extension bundle, which ships with no dependencies.
 *   SMALL         one glob compiler, no minimatch. Adding a dependency to the
 *                 extension means a bundler; see the offline .vsix packager.
 *   ORDERED       ALWAYS rules are evaluated first and cannot be negated. A
 *                 user's allow-rule can re-open something THEY excluded; it can
 *                 never re-open a credential file.
 *
 * PATH CONTRACT
 *
 * Callers pass a workspace-RELATIVE path with forward slashes ("src/app.ts").
 * `normalizePath` tolerates backslashes, a leading "./" and a leading "/".
 *
 * Absolute paths are not rejected, and that is on purpose: every ALWAYS
 * pattern is basename-style, so `/etc/ssl/private/server.key` and
 * `C:\Users\me\.ssh\id_rsa` still match even if a call site forgets to relativise
 * first. Anchored rules (those containing a slash) will not match an absolute
 * path, which is correct — they are defined relative to the workspace root.
 *
 * MATCHING SEMANTICS (gitignore-shaped, and intentionally so — people already
 * know this syntax and most projects already have a .gitignore to paste from):
 *
 *   node_modules      no slash  -> matches that BASENAME at any depth, plus
 *                                  everything beneath it if it is a directory
 *   src/generated     a slash   -> anchored at the workspace root
 *   build/            trailing  -> directories only
 *   *.pem             *         -> any run of characters except "/"
 *   **                two stars -> crosses directory separators. Followed by a
 *                                  slash it also matches ZERO segments, so a
 *                                  two-star prefix reads as "at any depth".
 *   ?                           -> exactly one character except "/"
 *   [abc] [!abc]                -> character class, negated class
 *   !keep/this.md     leading ! -> re-allow (last match wins), except ALWAYS
 *   # comment                   -> ignored, so a .gitignore pastes in cleanly
 *
 * Matching is CASE-INSENSITIVE. Windows and macOS default to case-insensitive
 * filesystems, so a rule for `.env` that let `.ENV` through would be a rule
 * that protects nothing on two of the three platforms this runs on.
 * ------------------------------------------------------------------------- */
...
/** Where a rule came from. Drives both the UI grouping and the refusal text. */
export type RuleOrigin = "always" | "group" | "user";

export interface ExclusionGroup {
  id: string;
  label: string;
  /** One line, shown beside the toggle. */
  blurb: string;
  patterns: string[];
}

/** What is persisted per user. Everything else is derived from this. */
export interface ExclusionConfig {
  /** The user's own patterns, in order. May contain "!" negations. */
  patterns: string[];
  /** Ids from RECOMMENDED_GROUPS the user has switched OFF. */
  disabledGroups: string[];
  updatedAt: number;
}

export interface CompiledRule {
  /** The pattern exactly as written, for display and for error messages. */
  pattern: string;
  negated: boolean;
  dirOnly: boolean;
  origin: RuleOrigin;
  ...
  reSelf: RegExp;
  /** Matches anything beneath the path. */
  reUnder: RegExp;
}

export interface ExclusionDecision {
  excluded: boolean;
  /** The rule that decided it, present whenever one matched at all. */
  rule?: CompiledRule;
  /** A sentence safe to show the user or hand back to the model. */
  reason?: string;
}
 ...
 */
export const ALWAYS_PATTERNS: string[] = [
 ...
 */
export const ALWAYS_EXCEPTIONS: string[] = [
  ".env.example",
  ".env.sample",
  ".env.template",
  ".env.dist",
 ...
 */
export const RECOMMENDED_GROUPS: ExclusionGroup[] = [
  {
    id: "deps",
    label: "Dependencies",
...
export const RECOMMENDED_GROUP_IDS = RECOMMENDED_GROUPS.map((g) => g.id);
...
export const MAX_USER_PATTERNS = 300;
...
function ruleMatches(rule: CompiledRule, path: string, isDir: boolean): boolean {
  if (rule.reUnder.test(path)) return true;
...
export class ExclusionMatcher {
   ...
   */
  readonly always: CompiledRule[];
  readonly exceptions: CompiledRule[];
  ...
  constructor(config: ExclusionConfig) {
    ...
    this.always = compileList(ALWAYS_PATTERNS, "always");
    this.exceptions = compileList(ALWAYS_EXCEPTIONS, "always");
   ...
   * decide.
   */
  decide(path: string, isDir = false): ExclusionDecision {
    const p = normalizePath(path);
    if (!p) return { excluded: false };
      ...
      for (const rule of this.always) {
        if (ruleMatches(rule, p, isDir)) {
          return {
            ...
            rule,
            reason:
              `"${p}" is blocked by a built-in credential rule (${rule.pattern}). ` +
    ...
    let verdict: CompiledRule | null = null;
    for (const rule of this.rules) {
      if (ruleMatches(rule, p, isDir)) verdict = rule;
    }

    if (!verdict || verdict.negated) return { excluded: false, rule: verdict ?? undefined };
  ...
  promptSummary(): string {
    const groups = RECOMMENDED_GROUPS.filter(
      (g) => !(this.config.disabledGroups ?? []).includes(g.id),
    ...
    if (groups.length) lines.push(`- These are hidden from listings and reads: ${groups.join(", ")}.`);
    const userPatterns = (this.config.patterns ?? []).filter(
    ...
    lines.push(
      "- A refusal here is final. Do not retry the same file by another tool or another spelling; ask the user for what you need instead.",
    );
    return lines.join("\n");
  }
}
...
function groupLabel(id?: string): string {
  return RECOMMENDED_GROUPS.find((g) => g.id === id)?.label ?? id ?? "recommended";
}
...
export function validatePattern(pattern: string): string | null {
  ...
  if (raw.length > MAX_PATTERN_LENGTH) {
    return `Too long (max ${MAX_PATTERN_LENGTH} characters).`;
  }
  if (raw.includes("\0")) return "Contains a null byte.";
  if ((raw.match(/[*?]/g) || []).length > MAX_WILDCARDS) {
    return `Too many wildcards (max ${MAX_WILDCARDS}).`;
  }
  const trimmed = raw.trim();
  if (!trimmed || trimmed.startsWith("#")) return null; // blank/comment: dropped, not an error
  if (trimmed === "!") return "A rule cannot be just \"!\".";
  try {
...
export interface SanitizedConfig {
  ...
  /** Per-pattern complaints, keyed by the pattern as submitted. */
 ...
 */
export function sanitizeConfig(input: unknown): SanitizedConfig {
  ...
  for (const entry of rawPatterns) {
    ...
    if (patterns.length >= MAX_USER_PATTERNS) {
      errors.push({
        pattern: entry,
        error: `Dropped — the list is capped at ${MAX_USER_PATTERNS} rules.`,
      });
      continue;
    }
  ...
  const disabledGroups = rawGroups
    ...
    .filter((g) => RECOMMENDED_GROUP_IDS.includes(g));
...
export function matcherFrom(config: Partial<ExclusionConfig> | null | undefined): ExclusionMatcher {
 ...
 */
export const DEFAULT_MATCHER = matcherFrom(EMPTY_EXCLUSION_CONFIG);

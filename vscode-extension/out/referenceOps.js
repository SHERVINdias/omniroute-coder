...
exports.attachConsent = attachConsent;
exports.getReferenceRoot = getReferenceRoot;
exports.isReferenceFolder = isReferenceFolder;
exports.setReferenceRoot = setReferenceRoot;
exports.refListFiles = refListFiles;
exports.refReadFile = refReadFile;
exports.refSearch = refSearch;
 ...
 */
function isReferenceFolder(fsPath) {
    if (!referenceRoot)
        return false;
...
function openFolderFor(fsPath) {
    const folders = vscode.workspace.workspaceFolders || [];
    for (const folder of folders) {
        if (samePath(folder.uri.fsPath, fsPath))
            return folder;
    }
    return null;
}
 ...
 */
function setReferenceRoot(input) {
 ...
 */
function requireReferenceRoot() {
    if (!referenceRoot) {
        throw new Error("No reference project is configured. Choose one in the web app under " +
            "Projects, then try again.");
    }
    const folder = openFolderFor(referenceRoot.path);
    if (!folder) {
        throw new Error(`The reference project "${referenceRoot.name}" is not open in this VS Code window. ` +
    ...
    if (!consent?.isGranted(folder.uri.fsPath)) {
        consent?.promptInBackground(folder);
        throw new Error(`OmniRoute is not approved to read "${referenceRoot.name}" yet. A prompt should be ` +
 ...
 */
function resolveInside(root, relativePath, isDir = false) {
    ...
    if (back.startsWith("..") || path.isAbsolute(back)) {
        throw new Error(`File access blocked: "${relativePath}" is outside the reference project. ` +
            "Only paths inside the approved folder can be read.");
    }
    if (rel) {
        const verdict = (0, fileOps_1.getExclusionMatcher)().decide(rel, isDir);
        if (verdict.excluded) {
...
function includeFor(relDir, pattern) {
 ...
 * list
 * ---------------------------------------------------------------------- */
async function refListFiles(options) {
    ...
    const relDir = String(options?.dir ?? "")
        ...
        .replace(/\/+$/, "");
    if (relDir)
        resolveInside(root, relDir, true);
    const depth = Math.max(1, Math.min(12, Number(options?.depth) || 3));
    const matcher = (0, fileOps_1.getExclusionMatcher)();
    const found = await vscode.workspace.findFiles(new vscode.RelativePattern(vscode.Uri.file(root), includeFor(relDir, options?.pattern)), (0, fileOps_1.buildPruneGlob)(matcher), MAX_FIND_RESULTS);
    const entries = [];
    const seenDirs = new Set();
    let truncated = found.length >= MAX_FIND_RESULTS;
    for (const uri of found) {
        const rel = path.relative(root, uri.fsPath).replace(/\\/g, "/");
        if (!rel || matcher.test(rel, false))
            continue;
        ...
        if (segments.length > depth) {
            ...
            for (let i = 0; i < depth && i < segments.length - 1; i++) {
                ...
                if (seenDirs.has(prefix) || matcher.test(prefix, true))
                    continue;
                seenDirs.add(prefix);
 ...
 * read
 * ---------------------------------------------------------------------- */
async function refReadFile(options) {
    const root = requireReferenceRoot();
    ...
    if (stat.size > MAX_READ_BYTES) {
        throw new Error(`"${rel}" is ${Math.round(stat.size / 1024)} KB, too large to read from the reference ` +
    ...
    const end = Math.min(lines.length, Math.floor(Number(options.endLine) || lines.length));
    return {
        path: rel.replace(/\\/g, "/"),
 ...
 */
async function refSearch(options) {
    const root = requireReferenceRoot();
    const query = String(options?.query ?? "").trim();
    if (!query) {
        throw new Error("Missing 'query' parameter for ref_search.");
    }
    const limit = Math.max(1, Math.min(MAX_SEARCH_RESULTS, Number(options?.maxResults) || 30));
    const caseSensitive = options?.caseSensitive === true;
    const needle = caseSensitive ? query : query.toLowerCase();
    const matcher = (0, fileOps_1.getExclusionMatcher)();
    const found = await vscode.workspace.findFiles(new vscode.RelativePattern(vscode.Uri.file(root), includeFor("", options?.pattern)), (0, fileOps_1.buildPruneGlob)(matcher), MAX_FIND_RESULTS);
    const matches = [];
    let scanned = 0;
    let truncated = false;
    for (const uri of found) {
        ...
        try {
            const stat = await vscode.workspace.fs.stat(uri);
            if (stat.size > MAX_SEARCHABLE_BYTES)
                continue;
            const bytes = await vscode.workspace.fs.readFile(uri);
...
//# sourceMappingURL=referenceOps.js.map
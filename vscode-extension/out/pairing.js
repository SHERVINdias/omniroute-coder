...
exports.PairingStore = exports.LOCAL_BRIDGE_URL = void 0;
exports.looksLikeToken = looksLikeToken;
exports.normaliseBridgeUrl = normaliseBridgeUrl;
exports.isUnencryptedRemote = isUnencryptedRemote;
exports.parsePairingInput = parsePairingInput;
exports.encodePairingLink = encodePairingLink;
...
exports.LOCAL_BRIDGE_URL = "ws://127.0.0.1:20129";
/* ------------------------------ validation ------------------------------ */
/**
...
function normaliseBridgeUrl(raw) {
    ...
    try {
        ...
        throw new Error(`"${trimmed}" is not a URL. It should look like wss://your-host/vscode-bridge, ` +
            `or ${exports.LOCAL_BRIDGE_URL} if OmniRoute is running on this machine.`);
    }
    if (parsed.protocol === "http:" || parsed.protocol === "https:") {
        const suggested = (parsed.protocol === "https:" ? "wss://" : "ws://") +
            parsed.host +
            (parsed.pathname === "/" ? "/vscode-bridge" : parsed.pathname);
        throw new Error(`That is the web address of OmniRoute, not the bridge address. ` +
    ...
    if (parsed.protocol !== "ws:" && parsed.protocol !== "wss:") {
        throw new Error(`"${parsed.protocol}" is not a WebSocket scheme. The bridge address must start with wss:// or ws://.`);
    }
 ...
 */
function isUnencryptedRemote(url) {
    try {
...
function fromJsonBody(raw) {
    ...
    const token = [record.token, record.t, record.pairingToken].find((v) => typeof v === "string" && v);
    if (!token)
        return null;
    const url = [record.wsUrl, record.url, record.u, record.serverUrl].find((v) => typeof v === "string" && v);
 ...
 */
function decodeLinkPayload(candidate) {
    ...
    for (let end = candidate.length; end >= FLOOR; end--) {
        ...
        if (parsed && looksLikeToken(parsed.token))
            return parsed;
    }
    return null;
}
 ...
 */
function parsePairingInput(input) {
    let raw = (input || "").trim();
    ...
    if (linkAt !== -1) {
        ...
        if (!parsed) {
            throw new Error("That pairing code could not be read. Copy it again from the \"Connect VS Code\" panel in " +
    ...
    if (raw.startsWith("{")) {
        ...
        if (parsed && looksLikeToken(parsed.token))
            return parsed;
        throw new Error("That looks like JSON, but it has no `token` field containing an omr_pair_… value.");
    }
    ...
    if (urlMatch) {
        ...
        if (!token) {
            throw new Error("That address has no pairing token in it. Use the full pairing code from OmniRoute " +
    ...
    if (tokenAt !== -1) {
        ...
        if (!looksLikeToken(cleaned)) {
            ...
            throw new Error(`That token is ${body.length} characters after the prefix; a whole one is ${TOKEN_BODY_LENGTH}. ` +
                `It was probably cut short when it was copied.`);
        }
        return { url: null, token: cleaned };
    }
    throw new Error("That does not look like an OmniRoute pairing code. A pairing code starts with omr_link_ " +
 ...
 */
function encodePairingLink(pairing) {
...
class PairingStore {
    ...
    async migrateLegacySetting() {
        ...
        const legacyToken = (config.get(LEGACY_TOKEN_SETTING) || "").trim();
        if (!legacyToken)
            return null;
        const legacyUrl = (config.get(LEGACY_URL_SETTING) || "").trim();
        let pairing;
        try {
            const parsed = parsePairingInput(legacyToken);
            pairing = {
                token: parsed.token,
                url: parsed.url || (legacyUrl ? normaliseBridgeUrl(legacyUrl) : exports.LOCAL_BRIDGE_URL),
            };
        }
        catch {
     ...
     */
    async clearLegacySetting() {
        const config = vscode.workspace.getConfiguration("omniroute");
        const inspected = config.inspect(LEGACY_TOKEN_SETTING);
        const targets = [

...
const referenceOps_1 = require("./referenceOps");
let client = null;
let consentStore = null;
let pairingStore = null;
let statusBarItem;
function activate(context) {
    consentStore = new consent_1.ConsentStore(context);
    pairingStore = new pairing_1.PairingStore(context);
    client = new wsClient_1.OmniRouteWSClient(context);
    /* ---- file ops resolve their root through consent, from here on ---- */
    (0, fileOps_1.setRootResolver)(() => resolveConsentedRoot(consentStore));
     ...
     * configuration time. */
    (0, referenceOps_1.attachConsent)(consentStore);
    /* ---------------------------- status bar ---------------------------- */
     ...
     * reconnect. */
    context.subscriptions.push(consentStore.onDidChange(() => renderStatus(client.currentStatus())));
    /* ------------------------- the RPC dispatcher ------------------------ */
    client.setMessageHandler(async (message) => {
        ...
        switch (action) {
            ...
            case "list_files": {
                ...
                const files = await (0, fileOps_1.listFiles)(dir);
                return { files };
            }
            ...
            case "get_workspace_path":
                return (0, fileOps_1.getWorkspaceRoot)();
            ...
            case "read_file": {
                ...
                return await (0, fileOps_1.readFile)(filePath);
            }
            case "write_file": {
                const filePath = payload?.path || payload?.file_path || payload?.filePath;
                if (!filePath || payload?.content === undefined) {
                    throw new Error("Missing 'path' or 'content' parameter for write_file operation.");
                }
                return await (0, fileOps_1.writeFile)(filePath, payload.content);
            }
            case "replace_text": {
                const filePath = payload?.path || payload?.file_path || payload?.filePath;
                const oldText = payload?.old_text ?? payload?.oldText ?? payload?.search ?? payload?.findText;
                const newText = payload?.new_text ?? payload?.newText ?? payload?.replace ?? payload?.replaceText;
                const replaceAll = payload?.replace_all === true || payload?.replaceAll === true;
                if (!filePath || oldText === undefined || newText === undefined) {
                    throw new Error("Missing 'path', 'old_text', or 'new_text' parameter for replace_text operation.");
                }
                return await (0, fileOps_1.replaceText)(filePath, oldText, newText, replaceAll);
            }
            /* ------------------------ access rules ------------------------- */
            ...
            case "set_file_exclusions": {
                ...
                return (0, fileOps_1.setExclusionConfig)({
            ...
            case "set_reference_project":
                return (0, referenceOps_1.setReferenceRoot)({
                    path: payload?.path ?? null,
                    name: payload?.name ?? null,
                });
            case "ref_list_files":
                return await (0, referenceOps_1.refListFiles)({
                    dir: payload?.path ?? payload?.dir ?? "",
                    depth: payload?.depth,
                    pattern: payload?.pattern,
                });
            case "ref_read_file":
                return await (0, referenceOps_1.refReadFile)({
            ...
            case "ref_search":
                return await (0, referenceOps_1.refSearch)({
...
async function bootstrap(context) {
    try {
        ...
        if (rescued) {
            void vscode.window.showInformationMessage("OmniRoute: your pairing code has been moved out of settings.json into secure storage.");
        }
    }
    catch (err) {
 ...
 */
async function offerFirstRunGuidanceOnce(context) {
    if (context.globalState.get(FIRST_RUN_KEY))
        return;
    await context.globalState.update(FIRST_RUN_KEY, true);
    const choice = await vscode.window.showInformationMessage("OmniRoute is installed but not connected yet. Open OmniRoute in your browser, " +
        'go to "Connect VS Code" and click "Open in VS Code" — it pairs this window for you.', "Paste a code instead");
 ...
 */
async function reportDuplicateInstall(context, collisions) {
    const twins = vscode.extensions.all.filter((ext) => ext.id !== context.extension.id &&
        ext.id.toLowerCase().endsWith(".omniroute-vscode"));
    if (twins.length === 0 && collisions.length === 0)
        return;
    if (context.globalState.get(DUPLICATE_WARNED_KEY))
        return;
    await context.globalState.update(DUPLICATE_WARNED_KEY, true);
    const names = twins.map((ext) => ext.id).join(", ") || "an older build";
    const choice = await vscode.window.showWarningMessage(`Another copy of the OmniRoute extension is installed (${names}). Two copies fight ` +
        "over the same commands, which is why the status bar item can spin forever or report " +
        "a missing command. Uninstall the older one, then reload the window.", "Show me");
    if (choice === "Show me") {
 ...
 */
function resolveConsentedRoot(store) {
    const writable = store
        .grantedFolders()
        .map((folder) => path.resolve(folder.uri.fsPath))
        .filter((fsPath) => !(0, referenceOps_1.isReferenceFolder)(fsPath));
    ...
    if (store.grantedFolders().length > 0) {
        throw new Error("The only folder approved for OmniRoute is your read-only reference project, which can " +
    ...
    if (folders.length === 0) {
        throw new Error("No folder is open in the VS Code window connected to your account. " +
            "Open the project you want me to work in (File → Open Folder), then approve it for OmniRoute.");
    }
    nudgeConsent(store);
    throw new Error("OmniRoute is not approved to access this folder yet. A prompt should be visible in VS Code — " +
...
async function signInFlow() {
    ...
    let parsed;
    try {
        parsed = (0, pairing_1.parsePairingInput)(input);
    }
    catch (err) {
 ...
 */
async function handlePairingUri(uri) {
    const route = (uri.path || "").replace(/^\/+/, "").toLowerCase();
    if (route && route !== "pair") {
        void vscode.window.showWarningMessage(`OmniRoute: that link asked for “/${route}”, which this extension does not handle.`);
        return;
    }
    ...
    if (!code) {
        void vscode.window.showErrorMessage("OmniRoute: that link did not carry a pairing code. Open the “Connect VS Code” panel in OmniRoute and try again.");
        return;
    }
    let parsed;
    try {
        parsed = (0, pairing_1.parsePairingInput)(code);
    }
    catch (err) {
        void vscode.window.showErrorMessage(`OmniRoute: ${err?.message || "the pairing code in that link could not be read."}`);
        return;
    }
 ...
 */
async function applyPairing(parsed, opts) {
    let url = parsed.url;
    if (!url) {
        const entered = await vscode.window.showInputBox({
            ...
            value: pairing_1.LOCAL_BRIDGE_URL,
            ignoreFocusOut: true,
            validateInput: (value) => {
                try {
                    (0, pairing_1.normaliseBridgeUrl)(value);
                    return null;
                }
                catch (err) {
        ...
        if (entered === undefined)
            return;
        try {
            url = (0, pairing_1.normaliseBridgeUrl)(entered);
        }
        catch (err) {
    ...
    const risky = (0, pairing_1.isUnencryptedRemote)(url);
    if (opts.confirmFirst || risky) {
        const detail = [
            ...
            .filter(Boolean)
            .join("\n\n");
        const proceed = await vscode.window.showWarningMessage(`Connect this editor to OmniRoute at ${describeTarget(url)}?`, { modal: true, detail }, "Connect");
        if (proceed !== "Connect")
            return;
    }
    ...
    if (opts.confirmFirst && client.currentStatus().kind === "connected") {
        void vscode.window.showInformationMessage("OmniRoute: this editor is paired and connected.");
    }
}
...
async function offerFolderApprovalOnce() {
    if (!consentStore)
        return;
    if (consentStore.grantedFolders().length > 0)
        return;
    const folders = vscode.workspace.workspaceFolders || [];
    if (folders.length === 0)
        return;
...
async function signOutFlow() {
    const confirm = await vscode.window.showWarningMessage("Sign out of OmniRoute? Your pairing code will be removed from this machine and you will need a new one to reconnect.", { modal: true }, "Sign out");
    if (confirm !== "Sign out")
        return;
 ...
 */
async function manageFoldersFlow() {
    if (!consentStore)
        return;
    ...
    if (picked.action === "revokeAll") {
        ...
        void vscode.window.showInformationMessage(n > 0 ? `OmniRoute: cleared ${n} folder approval${n === 1 ? "" : "s"}.` : "OmniRoute: nothing to clear.");
        return;
    }
    if (picked.action === "approve" && picked.fsPath) {
        await consentStore.grant(picked.fsPath, picked.name);
        if (client?.currentStatus().kind !== "connected")
            void client?.connect();
        return;
    }
...
function describeStatus(status, paired) {
    if (!paired)
        return "Not connected to any OmniRoute account.";
    switch (status.kind) {
        case "connected":
            return "Connected.";
        case "connecting":
...
function deactivate() {
    ...
    (0, fileOps_1.setRootResolver)(null);
     ...
     * give. */
    (0, referenceOps_1.attachConsent)(null);
}
